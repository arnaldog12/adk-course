...
def check_agent_availability(agent_name: str, base_url: str) -> dict:
    ...
    try:
        ...
        return {"status": "error", "available": False, "report": f"Failed to check {agent_name}: {e!s}"}
...
def log_coordination_step(step: str, agent_name: str = "") -> dict:
    ...
    return {"status": "success", "report": message, "step": step, "agent": agent_name}
...
Always explain which remote agent you're delegating to and why.
    ...
    tools=[FunctionTool(check_agent_availability), FunctionTool(log_coordination_step)],
    generate_content_config=types.GenerateContentConfig(temperature=0.5, max_output_tokens=2048),
)
