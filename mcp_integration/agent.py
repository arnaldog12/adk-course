...
def before_tool_callback(tool, args: dict[str, Any], tool_context) -> dict[str, Any] | None:
        ...
        if not auto_approve:
            ...
            return {
                ...
                "message": (
                    ...
                    f"Reason: {reason}\n"
                    f"Arguments: {args}\n\n"
                    f"To proceed, reply with one of:\n"
                    f"  • 'approve' — enable auto-approval for all file operations\n"
                    f"  • 'deny' — cancel this operation\n\n"
...
def set_file_ops_approval(enabled: bool, tool_context: ToolContext) -> dict[str, Any]:
    """Enable or disable auto-approval for destructive file operations.

    When enabled, write/move/create_directory operations are executed immediately
    without asking for confirmation each time. When disabled (default), those
    operations are blocked until the user explicitly approves each one.

    Args:
        enabled: True to enable auto-approval, False to require per-operation approval.
        tool_context: ADK ToolContext used to persist the preference in user state.

    Returns:
        dict with the updated approval status.
    """
    tool_context.state["user:auto_approve_file_ops"] = enabled
    status = "ENABLED" if enabled else "DISABLED"
    logger.info(f"[HITL] Auto-approval for file operations {status} by user request")
    return {
        "status": "ok",
        "auto_approve_file_ops": enabled,
        "message": (
            f"✅ Auto-approval for file operations is now {'enabled' if enabled else 'disabled'}.\n"
            + (
                "Write, move and create_directory operations will proceed without confirmation."
                if enabled
                else "Write, move and create_directory operations will require your approval each time."
            )
        ),
    }


def create_mcp_filesystem_agent(base_directory: str | None = None, enable_hitl: bool = True) -> Agent:
    ...
    if base_directory is None:
        ...
        base_directory = str(current_dir / "sample_files")
    ...
    server_params = StdioServerParameters(
        command="npx", args=["-y", "@modelcontextprotocol/server-filesystem", base_directory]
    )
...
- set_file_ops_approval: Enable or disable auto-approval for destructive operations
...
4. HUMAN-IN-THE-LOOP WORKFLOW:
    - Destructive operations (write, move, delete) require user approval by default
    - If a destructive operation is blocked, inform the user and offer two options:
      a) Approve this specific operation manually
      b) Call set_file_ops_approval(enabled=True) to enable auto-approval for all future operations
    - If the user says "approve", "yes", "allow", "enable auto approve" or similar
      → call set_file_ops_approval(enabled=True)
    - If the user says "disable auto approve", "require approval again" → call set_file_ops_approval(enabled=False)
...
You: "I'll create a file called hello.py with a hello world script. Here's what I'll write: [show content].
...
REMEMBER: You're an intelligent assistant, not a literal command parser. Understand intent, use context,
    and act decisively.
        """.strip(),
        tools=[mcp_tools, set_file_ops_approval],
